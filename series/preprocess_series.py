...
"""
for serie in series:
    data_cur=csv_to_data(serie+".csv", serie)
    data=data.append(data_cur,ignore_index=True)
data.to_csv("series8_data.csv", index=False)
#%%
#for val
import torch
...
total_dict={}
for serie in series:
    cur_text=txt_to_text(serie+".txt")
    ...
    for word in PERSONS:
        if len(word)==1:
            ...
            cur+=1
    total_dict[serie]=(replace_dict1,replace_dict2,replace_dict3)
...
data_replace2.to_csv("series8_data_replace2.csv", index=False)
#%%
...
for serie in series:
    ...
    cur_labels=[serie]*len(cur_sentences)
    data_cur=pd.DataFrame({"sentence":cur_sentences,"label":cur_labels})
    val=pd.concat([val,data_cur],ignore_index=True)
val.to_csv("series8_val.csv", index=False)
#%%
...
val_replace.to_csv("series8_val_replace.csv", index=False)
#%%
total_data=pd.concat([data_replace2,val],ignore_index=True)
total_data.to_csv("series8_total.csv", index=False)
#%%
"""
#augmen
